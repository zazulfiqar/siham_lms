...
@section('page_title', 'Edit Exams')
@section('content')
    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">Edit Examination</h6>
            {!! Qs::getPanelOptions() !!}
        </div>
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                <li class="nav-item"><a href="#new-course" class="nav-link active" data-toggle="tab">Edit your Uploaded Exam</a>
                </li>
            </ul>
...
{{--                            <form class="" method="get" action="{{ route('exams.update',Qs::hash($exams->id)) }}" enctype="multipart/form-data">--}}
                            <form class="" method="post" action="{{ route('exams.update',Qs::hash($exams->id)) }}" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                    ...
                                    <div class="col-lg-9">
                                        <select required data-placeholder="Select Course" class="form-control select"
                                            ...
                                            @foreach($courses as $c)
                                                <option
                                                    {{ $exams->course_id == $c->id ? 'selected' : '' }} value="{{ $c->id }}">{{ $c->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-lg-9">
                                        <select required data-placeholder="Select Class" class="form-control select"
                                            ...
                                            @foreach($my_classes as $c)
                                                <option
                                                    {{ $exams->class_id == $c->id ? 'selected' : '' }} value="{{ $c->id }}">{{ $c->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
{{--                                                <option--}}
{{--                                                    {{ $exams->teacher_id == $t->user_id ? 'selected' : '' }} value="{{ $t->user_id }}">{{ $t->name }}</option>--}}
{{--                                            @endforeach--}}
{{--                                        </select>--}}
{{--                                    </div>--}}
{{--                                </div>--}}
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9">
                                        <input type="file" name="file" value="{{ $exams->file }}"  type="text"
                                ...
                                <div class="form-group row">
                                    <label for="name" class="col-lg-3 col-form-label font-weight-semibold">Topic
                                        <span class="text-danger">*</span></label>
                                    <div class="col-lg-9">
                                        <input id="code" name="topic" value="{{ $exams->topic }}" required type="text"
                                               class="form-control" placeholder="Course Topic">
                                    </div>
                                </div>
