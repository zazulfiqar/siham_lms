                        ...
                        @foreach($teachers as $teach)
                            <tr>
                                ...
                                <td><img class="rounded-circle" style="height: 40px; width: 40px;"
                                         src="{{url('/siham_lms/storage/app/public/'.$teach->user->photo	)}}" alt="photo"></td>
                                <td>{{ $teach->user->name }}</td>
                                {{--                                <td>{{ $teach->adm_no }}</td>--}}
                                {{--                                <td>{{ $my_class->name.' '.$teach->section->name }}</td>--}}
                                <td>{{ $teach->user->email }}</td>
                                <td>{{ $teach->speciality }}</td>
                                    ...
                                    <div class="list-icons">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-left">
                                                <a href="{{ route('teachers.show', Qs::hash($teach->id)) }}"
                                                   class="dropdown-item"><i class="icon-eye"></i> View Profile</a>
                                                @if(Qs::userIsTeamSA())
                                                    <a href="{{ route('teachers.edit', Qs::hash($teach->id)) }}"
                                                       class="dropdown-item"><i class="icon-pencil"></i> Edit</a>
                                                    <a href="{{ route('teacher.reset_pass', Qs::hash($teach->user->id)) }}"
                                                       class="dropdown-item"><i class="icon-lock"></i> Reset
                                                        password</a>
                                                @endif
                                                {{--                                                <a target="_blank" href="{{ route('marks.year_selector', Qs::hash($teach->user->id)) }}" class="dropdown-item"><i class="icon-check"></i> Marksheet</a>--}}

                                                {{--            Delete--}}
                                                @if(Qs::userIsSuperAdmin())
                                                    <a id="{{ Qs::hash($teach->user->id) }}"
                                                       onclick="confirmDelete(this.id)" href="#"
                                                       class="dropdown-item"><i class="icon-trash"></i> Delete</a>
                                                    <form method="post" id="item-delete-{{ Qs::hash($teach->user->id) }}"
                                                          action="{{ route('students.destroy', Qs::hash($teach->user->id)) }}"
